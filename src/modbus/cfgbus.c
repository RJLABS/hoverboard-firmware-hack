...
uint16_t (*CfgStore)(volatile uint16_t *data, uint16_t len) = &writeFlash;
uint16_t (*CfgLoad)(volatile uint16_t *data, uint16_t len)  = &readFlash;
...
void CfgInit()
{
  ...
  if(CfgLoad(cfg.regs,CFG_NR_REGISTERS) != CFG_NR_REGISTERS)
  {
...
int CfgPeek(uint8_t *out)
{
    short c = softwareserial_peekrx();
    if (c < 0){
        return 0;
    }
    *(out++) = c;
...
{
    return softwareserial_Send( data, len );
}


void CfgFlushRx()
{
    softwareserial_flushRX();
}
...
bool _isListEntry(uint16_t start, uint16_t nr_regs)
{
