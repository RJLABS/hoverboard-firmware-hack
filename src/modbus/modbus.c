...
int (*mb_peek)(uint8_t * data)                           = &CfgPeek;
...
void mb_do_timeout(uint8_t nextState)
{
  mb_error(mb_timeout);
  _state = nextState;
}
...
int mb_update() {
  ...
  {
    mb_peek(&_hdr.slave); //get slave address
    if(_hdr.slave != MB_SLAVE_ID && _hdr.slave != MB_BROADCAST_ADDR )
    {
      // SH - don't flush if we don;t get a 0x10 (our addr)
      // something else might be reading this serial
